...
maps the region at a different virtual base address. The C++ program (``simple_ivshmem.cpp``)
...
def _run(target, label, args, results):
    rc, out = target.execute(f"cd {CWD} && {APP} {args} --pci")
...
def test_simple_shared_memory_read_write(target_a, target_b):
    ...
    pci-server (--pci), so no /dev/ivshmem0 guest driver is required.
    """
    results = {}
    threads = [
