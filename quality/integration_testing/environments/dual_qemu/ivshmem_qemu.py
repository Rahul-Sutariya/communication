...
# *******************************************************************************
"""A QEMU launcher that mirrors the upstream ``Qemu`` class but adds an ``ivshmem-plain``
device so multiple VMs can share one host backing file.

The upstream ``Qemu.__build_qemu_command`` is name-mangled and exposes no extension point
for extra ``-object`` / ``-device`` flags, so the command is rebuilt here. The launcher
logic (KVM detection, lifecycle) is kept identical on purpose to stay close to upstream.
"""
import logging
import os
import subprocess
...
class IvshmemQemu:
    ...
    def __init__(
        ...
        :param str cpu: CPU model to emulate.
    ...
    def __build_qemu_command(self):
        ...
        return (
            [
                f"{self.__qemu_path}",
                "--enable-kvm" if self._accelerator_support == "kvm" else " -accel tcg",
                "-smp",
