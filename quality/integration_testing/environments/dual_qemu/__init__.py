...
It is a thin extension of the upstream single-VM ``qemu`` plugin
...
def _wait_for_ssh(target, total_timeout: int = 180, interval: int = 3, stable_successes: int = 3):
    """Wait until the freshly booted QNX VM *stably* serves SSH.

    On first boot QNX ``sshd`` answers one early connection and then is briefly
    unavailable (~30s) while the network stack / sshd finish warming up, before
    becoming rock-solid. Returning on the very first success can therefore land in
    that transient window and make the upstream ``pre_tests_phase`` (only 5 retries)
    fail. To avoid this we require several *consecutive* successful round-trips,
    spaced by ``interval`` seconds, which guarantees we are past the fragile window.
    """
...
def _boot_vm(make_process, vm, *, boot_timeout: int = 100, max_boot_attempts: int = 3):
    ...
    A QNX guest very occasionally boots but its ``sshd`` never actually serves
    (QEMU's SLIRP resets the SSH connection). That state is unrecoverable for that
    boot, so rather than burning the full SSH timeout and failing the whole test, we
    stop the wedged guest and start it again, up to ``max_boot_attempts`` times.

    Returns the live ``(process, target)`` of the successful boot. The caller owns
    stopping the returned process.
    """
    last_error = None
...
def target_a(targets):
    """The first VM (producer side in the inter-VM shared-memory tests)."""
    return targets[0]


@pytest.fixture(scope="session")
def target_b(targets):
    """The second VM (consumer side in the inter-VM shared-memory tests)."""
    return targets[1]
