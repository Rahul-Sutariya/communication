...
def _targets(config, ivshmem_backend):
    ...
    with DualQemuProcess(
        ...
        # Uses the "stream" netdev backend (reconnect= on the connect side), which
        # transparently re-pairs the intervm link regardless of which VM restarts or
        # in what order -- no peer coordination needed here (see ivshmem_qemu.py).
        ...
        try:
            process_b.start()
            # Re-verify both VMs are still responsive (either may have gone quiet while the
            # other booted). The probes are read-only SSH checks on already-booted VMs, so
            # running them concurrently is safe; any restart needed to self-heal still runs
            # sequentially below to avoid the concurrent-boot wedge that sequential *booting*
