...
# *******************************************************************************
#
# Launch TWO QNX QEMU VMs that share a single ivshmem region, exactly like the
# `dual_qemu` plugin does -- but by hand, so you can SSH in and run the read/write
# example yourself.
#
# Usage:
...
IMG=${IMG:-bazel-bin/quality/integration_testing/environments/dual_qemu/example/init_ifs_test_simple_ivshmem}
...
if [[ ! -f "${IMG}" ]]; then
    ...
    echo "  bazel build --config=qnx_x86_64 //quality/integration_testing/environments/dual_qemu/example:_init_ifs_test_simple_ivshmem" >&2
    exit 1
fi
