...
Each VM is described by the *upstream* ``QemuConfigModel`` (so a dual config is just two
ordinary QEMU configs plus a shared ``ivshmem`` block), keeping this plugin trivially
upstreamable.
"""
import json
import logging
...
class InterVmNetwork(BaseModel):
    ...
    # Optional point-to-point socket NIC between the two VMs, used by the socket-based
    # control plane (PR #508). Off by default: the generic shared-memory test only needs
    # the ivshmem region plus host SSH.
    enabled: bool = False
