...
class QemuHypervisorTransportTest : public ::testing::Test
{
  public:
    ...
    QemuHypervisorTransportTest& WithAQemuHypervisorTransportWithEmptyPathResolver()
    {
        transport_ = std::make_unique<QemuHypervisorTransport>(
            ...
            [](const impl::InstanceSpecifier&) { return ShmPaths{"", ""}; });
        return *this;
    }
  ...
  protected:
    void SetUp() override
    {
        ...
        ivshmem_provider_mock_ =
            std::make_shared<qemu::ivshmem::IvshmemTypedMemoryProviderMock>();
...
TEST_F(QemuHypervisorTransportTest, DestructorCallsShutdown)
{
    EXPECT_CALL(*bi_directional_transport_mock_, Shutdown()).Times(1);
    {
        QemuHypervisorTransport transport(
            gateway_core_mock_, std::move(mock_transport_owner_), ivshmem_provider_mock_);
    }
    // Shutdown called once via destructor
}
...
// --- Inbound message handling: valid specifiers ---
...
TEST_F(QemuHypervisorTransportTest,
       OnMessageReceivedProvideServiceRequestWithInvalidSpecifierReturnsWithoutCoreCall)
{
    this->WithAQemuHypervisorTransport().WithARegisteredOnSetupCallback();
...
TEST_F(QemuHypervisorTransportTest,
       OnMessageReceivedStopOfferServiceRequestWithInvalidSpecifierReturnsWithoutCoreCall)
{
    this->WithAQemuHypervisorTransport().WithARegisteredOnSetupCallback();
...
TEST_F(QemuHypervisorTransportTest,
       OnMessageReceivedOfferServiceRequestWithInvalidSpecifierReturnsWithoutCoreCall)
{
    this->WithAQemuHypervisorTransport().WithARegisteredOnSetupCallback();
...
TEST_F(QemuHypervisorTransportTest,
       OnMessageReceivedUpdateNotificationWithValidSpecifierCallsNotifyUpdateOnGatewayCore)
{
    this->WithAQemuHypervisorTransport().WithARegisteredOnSetupCallback();
...
TEST_F(QemuHypervisorTransportTest,
       OnMessageReceivedUpdateNotificationWithInvalidSpecifierReturnsWithoutCoreCall)
{
    this->WithAQemuHypervisorTransport().WithARegisteredOnSetupCallback();
...
// --- PreCreateInterVmSharedMemory branch coverage ---
...
TEST_F(QemuHypervisorTransportTest, PreCreateInterVmSharedMemoryBindsCtrlAndDataWhenOffsetsFound)
{
    ...
    EXPECT_CALL(
        *ivshmem_provider_raw_, AllocateNamedTypedMemoryAtOffset(::testing::_, ::testing::_, 4096U, ::testing::_))
        .WillOnce(::testing::Return(score::cpp::expected_blank<score::os::Error>{}));
...
TEST_F(QemuHypervisorTransportTest, PreCreateInterVmSharedMemorySkipsCtrlBindWhenSizeIsZero)
{
    ...
    auto request = std::make_unique<ProvideServiceRequest>(
        specifier, std::vector<impl::EventInfo>{}, 0U, 4096U);
...
TEST_F(QemuHypervisorTransportTest, PreCreateInterVmSharedMemorySkipsDataBindWhenSizeIsZero)
{
    ...
    auto specifier = CreateValidInstanceSpecifier();
    auto request = std::make_unique<ProvideServiceRequest>(
        specifier, std::vector<impl::EventInfo>{}, 1024U, 0U);
