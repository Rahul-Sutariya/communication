...
def test_qemu_ivshmem_transport(target_a, target_b):
    ...
    # Both apps rendezvous via a handshake in the last page of the BAR.
    # Start both concurrently — app2 polls until app1 signals READY.
    threads = [
        threading.Thread(target=_run, args=(target_a, "VM-A (src)", APP1, results)),
        threading.Thread(target=_run, args=(target_b, "VM-B (dest)", APP2, results)),
    ]
    for t in threads:
        t.start()
    for t in threads:
    ...
    rc_a, text_a = results["VM-A (src)"]
    rc_b, text_b = results["VM-B (dest)"]
