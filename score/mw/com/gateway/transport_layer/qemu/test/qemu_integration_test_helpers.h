...
/// no retry — so a notification can be lost in transit (e.g. across a reconnect) with no error
/// surfaced on the sender. Resending is safe here: the peer's handler (TestGatewayCore::NotifyUpdate)
/// just (re-)sets an idempotent flag, so duplicates are harmless. Only the first attempt's failure
