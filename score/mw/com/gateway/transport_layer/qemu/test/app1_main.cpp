...
int main()
{
    ...
    std::fprintf(stderr, "app1: BAR paddr=0x%llx size=0x%llx\n",
    ...
    std::fprintf(stderr, "app1: waiting for service_b CTRL to appear in BAR directory...\n");
    {
        ...
        {
            std::fprintf(stderr, "app1: timed out waiting for service_b CTRL in directory\n");
            return 1;
        }
    }
    ...
    {
        std::fprintf(stderr, "app1: timed out waiting for service_b CTRL event_count\n");
        return 1;
    }
    ...
    {
        std::fprintf(stderr, "app1: service_b verification FAILED (magic=0x%08x d[1]=%u d[2]=%u)\n",
    ...
    {
        std::fprintf(stderr, "app1: timed out waiting for VM-B to verify service_a\n");
        return 1;
    }
